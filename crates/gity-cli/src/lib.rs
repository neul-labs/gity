...
fn render_metric_line(kind: JobKind, counts: gity_ipc::JobMetrics) -> String {
    format!(
...
mod tests {
    ...
    fn format_metrics_includes_counts() {
        ...
        let metrics = DaemonMetrics {
            jobs,
            global: gity_ipc::GlobalMetrics {
                pending_jobs: 1,
                uptime_seconds: 2,
                cpu_percent: 12.5,
                rss_bytes: 2048,
            },
            repos: vec![gity_ipc::RepoMetrics {
                repo_path: PathBuf::from("/tmp/demo"),
                pending_jobs: 3,
            }],
        };
