...
use gity_ipc::{DaemonCommand, DaemonResponse, DaemonService};
...
impl GityTray {
    ...
    pub fn new(config: TrayConfig) -> Result<Self, TrayError> {
        ...
        let tray = TrayIconBuilder::new()
            ...
            .with_tooltip("Gity - Git Helper Daemon")
            .with_icon(icon)
            .build()
...
fn handle_info_action(daemon_address: &str) {
    ...
    rt.block_on(async {
        let client = gity_daemon::NngClient::new(daemon_address.to_string());
        match client.execute(DaemonCommand::HealthCheck).await {
            Ok(DaemonResponse::Health(health)) => {
                ...
                show_notification("Gity Info", &info);
            }
            Ok(response) => {
                show_notification("Gity Info", &format!("Unexpected response: {:?}", response));
            }
            Err(e) => {
                show_notification("Gity Error", &format!("Daemon not running: {}", e));
            }
        }
    });
}
